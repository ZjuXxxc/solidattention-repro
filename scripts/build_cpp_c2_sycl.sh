...
if [[ ! -x "$compiler" ]]; then echo "missing oneAPI compiler: $compiler" >&2; exit 2; fi
mkdir -p "$build_dir"
"$compiler" -std=c++20 -O1 -pthread -fsycl \
  ...
  "$project_dir/cpp/src/selection_main.cpp" \
  ...
  -o "$build_dir/solidattention-c2-sycl"
echo "built $build_dir/solidattention-c2-sycl"
