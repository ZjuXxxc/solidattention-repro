...
"${compiler[@]}" -std=c++20 -O2 -pthread -Wno-nullability-completeness \
  -DSOLIDATTENTION_ENABLE_CUDA=1 \
  -I"$project_dir/cpp/include" \
  -I"$liburing/include" \
  -I"$cuda_headers" \
  -I"$cuda_runtime/include" \
  -I"$cuda_nvrtc/include" \
  "$project_dir/cpp/src/main.cpp" \
  "$project_dir/cpp/src/trace.cpp" \
  "$project_dir/cpp/src/uring_reader.cpp" \
  "$project_dir/cpp/src/cuda_backend.cpp" \
  ...
  -Wl,-rpath,"$cuda_runtime/lib" \
  -Wl,-rpath,"$cuda_nvrtc/lib" \
  -ldl -o "$build_dir/solidattention-c0"

echo "built $build_dir/solidattention-c0"
