...
"${compiler[@]}" -std=c++20 -O2 -Wno-nullability-completeness \
  -I"$project_dir/cpp/include" -I"$project_dir/cpp/src" -I"$liburing/include" \
  -I"$cuda_headers" -I"$cuda_runtime/include" -I"$cuda_nvrtc/include" \
  "$project_dir/cpp/src/qwen_chain_main.cpp" \
  "$project_dir/cpp/src/uring_reader.cpp" "$project_dir/cpp/src/trace.cpp" \
  ...
  -Wl,-rpath,"$cublas/lib" -ldl -o "$build_dir/solidattention-p1-2f"
echo "built $build_dir/solidattention-p1-2f"
